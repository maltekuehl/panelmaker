    ...
    try {
      const res = await fetch(`/api/panels/${panelId}/markers`, {
        ...
        body: JSON.stringify({
          markerId: marker.id,
          antibodyId: ab.id,
          ...(ab.conjugate && !marker.fluorophore ? { fluorophore: ab.conjugate } : {}),
        }),
      })
      if (!res.ok) {
      ...
      <div className="flex justify-between items-start">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-1">
            ...
            {(fluorophore || hostOrganism) && (
              <p className="text-xs text-muted-foreground">{[fluorophore, hostOrganism].filter(Boolean).join(" • ")}</p>
            )}
            {antibodyName ? (
              <>
                <p className="text-[10px] text-zinc-500 truncate max-w-[180px]">{antibodyName}</p>
                {marker.antibody?.cloneId && (
                  <p className="text-[10px] text-zinc-400 truncate max-w-[180px]">Clone: {marker.antibody.cloneId}</p>
                )}
              </>
            ) : (
              <Popover open={abOpen} onOpenChange={setAbOpen}>
                <PopoverTrigger asChild>
                  <button
                    ...
                    disabled={isUpdating}
                  >
                    {isUpdating ? <Loader2 className="h-3 w-3 animate-spin" /> : <FlaskConical className="h-3 w-3" />}
                    Choose antibody
